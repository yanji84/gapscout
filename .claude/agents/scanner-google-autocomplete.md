...
Every evidence item MUST have a `url` field where applicable. Output without URLs will be rejected by the citation pipeline.
...
For Google Autocomplete: No URL needed (autocomplete suggestions don't have URLs). For `webResearchFindings`, include the actual page URL from search results.
